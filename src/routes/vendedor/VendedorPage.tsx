...
export function VendedorPage() {
  ...
  // Pedidos devolvidos pelo escritório aguardando correção — o envio em si
  // fica na página Pedidos; aqui só o aviso pra não passar batido.
  const [pedidosDevolvidos, setPedidosDevolvidos] = useState(0)
  useEffect(() => {
    supabase
      .from('pedidos')
      .select('id', { count: 'exact', head: true })
      .eq('status', 'devolvido')
      .then(({ count }) => setPedidosDevolvidos(count ?? 0))
  }, [])
  ...
  return (
    ...
    >
      ...
      {pedidosDevolvidos > 0 && (
        <Link
          to="/vendedor/pedidos"
          className="mb-lg flex items-center gap-sm rounded-xl border border-error/30 bg-error/5 p-md transition-colors hover:bg-error/10"
        >
          ...
          <span className="flex-1 font-body-md text-body-md text-on-surface">
            Você tem {pedidosDevolvidos} pedido{pedidosDevolvidos === 1 ? '' : 's'} devolvido
            {pedidosDevolvidos === 1 ? '' : 's'} aguardando correção.
          </span>
          <span className="font-label-md text-label-md text-error">Ver pedidos</span>
        </Link>
      )}
